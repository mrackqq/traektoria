...
test.describe('Анкета', () => {
  ...
  test.fixme('Заполненная анкета применяется и создаёт расчёт', async ({ page }) => {
    ...
    await clickAction(page, page.getByRole('button', { name: 'Применить анкету' }));

    await expect(page.getByRole('link', { name: 'Посмотреть диагностику' })).toBeVisible({
      timeout: 60_000,
    });
  });
});
...
test.describe('Цель и маршрут', () => {
  ...
  test.fixme('Программа выбирается целью и ведёт к маршруту', async ({ page }) => {

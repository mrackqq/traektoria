...
test.describe('Размер целей нажатия на телефоне', () => {
  ...
  test('Ни один элемент управления не меньше собственного норматива 44px', async ({
    ...
    // Известный дефект, записанный в план переработки: названия программ
    // на «Моей цели» и в сравнении — обычные текстовые ссылки высотой
    // 22px. Чинится не отступом, а перестройкой этих экранов
    // (UI_UX_REDESIGN_PLAN.md, M4 и M7), поэтому пока вынесено в
    // исключение. Проверка продолжает ловить всё остальное, включая новые
    // нарушения на тех же экранах.
    const known = (v: TooSmall) =>
      (v.screen === '/goals' || v.screen === '/compare') && v.label.startsWith('a.');
